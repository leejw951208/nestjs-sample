...
async function main() {
    ...
    if (env === 'local' || env === 'dev') {
        migrationName = (await rl.question('파일명: ')).trim();
    }
    rl.close();
    ...
    if ((env === 'local' || env === 'dev') && !migrationName) {
        console.error('❌ 파일명을 입력해야 합니다.');
        process.exit(1);
    }
    ...
    try {
        if (env === 'local' || env === 'dev') {
            console.log(`🚀 ${env} 환경에서 마이그레이션을 실행합니다: ${migrationName}`);
            execSync(`npx prisma migrate dev --name ${migrationName} --schema=./prisma`, { stdio: 'inherit' });
        } else if (env === 'prod') {
        ...
        console.error('❌ 마이그레이션 실행 중 오류가 발생했습니다.');
        process.exit(1);
    }
}
